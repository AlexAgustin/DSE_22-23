...
void _ISR_NO_PSV _U2RXInterrupt()
{
    ...
    switch (caracter){ // switch de comprobacion del caracter recibido
        ...
        case 'R': //si es r o R, se mueve el servo incrementando el valor de duty (+10) si se respetan los limites (<= DUTY_MAX) y si flag_DUTY == 1 (se gestiona duty por UART)
            if(flag_DUTY && OC1RS+10<=DUTY_MAX) { // antes de mover el servo, se comprueba si al moverlo se seguiria dentro de los limites y si flag_DUTY == 1 (se gestiona duty por UART)
               OC1RS+=10;  //Incrementar el valor de duty: +10
        ...
        case 'L': //si es l o L, se mueve el servo decrementando el valor de duty (-10) si se respetan los limites (>= DUTY_MIN) y si flag_DUTY == 1 (se gestiona duty por UART)
            if(flag_DUTY && OC1RS-10>=DUTY_MIN) { // antes de mover el servo, se comprueba si al moverlo se seguiria dentro de los limites y si flag_DUTY == 1 (se gestiona duty por UART)
                OC1RS-=10; // Decrementar el valor de duty: -10
